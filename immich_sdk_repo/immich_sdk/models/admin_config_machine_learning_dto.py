...
class AdminConfigMachineLearningDto(BaseModel):
    ...
    """ # noqa: E501
    availability_checks: AdminConfigMachineLearningAvailabilityChecksDto = Field(alias="availabilityChecks")
    clip: AdminConfigClipDto
    duplicate_detection: AdminConfigDuplicateDetectionDto = Field(alias="duplicateDetection")
    enabled: StrictBool = Field(description="Enabled")
    facial_recognition: AdminConfigFacialRecognitionDto = Field(alias="facialRecognition")
    ocr: AdminConfigOcrDto
    urls: Annotated[List[StrictStr], Field(min_length=1)] = Field(description="ML service URLs")
    __properties: ClassVar[List[str]] = ["availabilityChecks", "clip", "duplicateDetection", "enabled", "facialRecognition", "ocr", "urls"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "availabilityChecks": AdminConfigMachineLearningAvailabilityChecksDto.from_dict(obj["availabilityChecks"]) if obj.get("availabilityChecks") is not None else None,
            "clip": AdminConfigClipDto.from_dict(obj["clip"]) if obj.get("clip") is not None else None,
            "duplicateDetection": AdminConfigDuplicateDetectionDto.from_dict(obj["duplicateDetection"]) if obj.get("duplicateDetection") is not None else None,
            "enabled": obj.get("enabled"),
            "facialRecognition": AdminConfigFacialRecognitionDto.from_dict(obj["facialRecognition"]) if obj.get("facialRecognition") is not None else None,
            "ocr": AdminConfigOcrDto.from_dict(obj["ocr"]) if obj.get("ocr") is not None else None,
            "urls": obj.get("urls")
        })
        return _obj
