...
class MemoryResponseDto(BaseModel):
    ...
    data: OnThisDayDto
    deleted_at: Optional[datetime] = Field(default=None, description="Deletion date", alias="deletedAt", json_schema_extra={"examples": ["2024-01-01T00:00:00Z"]})
    hide_at: Optional[datetime] = Field(default=None, description="Date when memory should be hidden", alias="hideAt", json_schema_extra={"examples": ["2024-01-01T00:00:00Z"]})
    id: UUID = Field(description="Memory ID")
    is_saved: StrictBool = Field(description="Is memory saved", alias="isSaved")
    memory_at: datetime = Field(description="Memory date", alias="memoryAt", json_schema_extra={"examples": ["2024-01-01T00:00:00Z"]})
    owner_id: UUID = Field(description="Owner user ID", alias="ownerId")
    seen_at: Optional[datetime] = Field(default=None, description="Date when memory was seen", alias="seenAt", json_schema_extra={"examples": ["2024-01-01T00:00:00Z"]})
    show_at: Optional[datetime] = Field(default=None, description="Date when memory should be shown", alias="showAt", json_schema_extra={"examples": ["2024-01-01T00:00:00Z"]})
    type: MemoryType
