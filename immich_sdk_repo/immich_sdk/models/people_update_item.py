...
class PeopleUpdateItem(BaseModel):
    ...
    PeopleUpdateItem
    """ # noqa: E501
    birth_date: Optional[date] = Field(default=None, description="Person date of birth", alias="birthDate")
    color: Optional[Annotated[str, Field(strict=True)]] = Field(default=None, description="Person color (hex)")
    feature_face_asset_id: Optional[UUID] = Field(default=None, description="Asset ID used for feature face thumbnail", alias="featureFaceAssetId")
    ...
    is_hidden: Optional[StrictBool] = Field(default=None, description="Person visibility (hidden)", alias="isHidden")
    ...
    def color_validate_regular_expression(cls, value):
        ...
        if isinstance(value, str) and not re.match(r"^#?([0-9A-Fa-f]{3}|[0-9A-Fa-f]{4}|[0-9A-Fa-f]{6}|[0-9A-Fa-f]{8})$", value):
            raise ValueError(r"must validate the regular expression /^#?([0-9A-Fa-f]{3}|[0-9A-Fa-f]{4}|[0-9A-Fa-f]{6}|[0-9A-Fa-f]{8})$/")
        return value
